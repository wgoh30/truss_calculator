...
from numpy import printoptions, degrees
...
def main() -> None:
    ...
    with printoptions(suppress=True, precision=3):
        for iteration in range(len(setupDict['force_matrix'])):
            print(f"{setupDict['force_list'][iteration].name}: {solvedForces[iteration][0]:.2f} N, {degrees(setupDict['force_list'][iteration].angle):.2f} deg")
